...
# --- 7. Config Generation (PERSISTENCE FIXED) ---
mkdir -p /home/steam/.config/sunshine

# Only overwrite if file is missing!
...
# ==============================================================================
# --- 9. THE SESSION MANAGER (Loop with Process Monitoring) ---
# ==============================================================================
# This loop handles Steam restarts. If Steam dies, we kill Gamescope and loop.
...
030000004c0500000000000000000000,PS5 Controller,a:b0,b:b1,back:b8,dpdown:h0.4,dpleft:h0.8,dpright:h0.2,dpup:h0.1,guide:b10,leftshoulder:b4,leftstick:b11,lefttrigger:a2,leftx:a0,lefty:a1,rightshoulder:b5,rightstick:b12,righttrigger:a5,rightx:a3,righty:a4,start:b9,x:b2,y:b3,platform:Linux,"

while true; do
    echo "--- Starting Session ---"
    
    # 9.1 Start Gamescope (Background)
    ...
    # 9.2 Wait for Wayland Socket (Needed for Sunshine to reconnect)
    TIMEOUT=20
    ...
    # 9.3 MONITOR STEAM
    # We loop here checking if Steam is alive. 
    # If Steam exits/crashes (pgrep fails), we kill Gamescope and loop to restart everything.
    
    # Give Steam 15 seconds to start up initially before monitoring
    sleep 15
    
    echo "Monitoring Steam process..."
    while kill -0 $GS_PID 2>/dev/null; do
        # Check if any steam process exists. 
        # If no steam process, it means we exited or crashed.
